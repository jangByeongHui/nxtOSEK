...
TASK(nxtcar)
{
	...
	sPIDControl=constrain(sPIDControl,-60,100);
	...
	lrPIDControl=constrain(lrPIDControl,-40,40);
...
TASK(MoveTask)
{
  while(1)
  {
	...
	if(temp_current<30&&lrPIDControl>0){
		nxt_motor_set_speed(NXT_PORT_B,(int)lrPIDControl,1);
		
	} else if(temp_current>-30&&lrPIDControl<0){
		nxt_motor_set_speed(NXT_PORT_B,(int)lrPIDControl,1);
	} else{
		nxt_motor_set_speed(NXT_PORT_B,0,1);
	}
